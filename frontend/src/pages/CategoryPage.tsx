...
import { fetchCategoryMovies } from '../services/ophimService'
    ...
    <div className="min-h-screen bg-slate-950">
      ...
      <main className="mx-auto w-full max-w-7xl px-4 pt-24 pb-6 md:px-8">
        <nav className="mb-6 flex items-center gap-2 text-sm text-slate-500">
          ...
          <span>/</span>
          <span className="text-slate-200">Thể loại: {title}</span>
        </nav>

        <div className="mb-8 flex items-center gap-3">
          <div className="h-7 w-1.5 rounded-full bg-gradient-to-b from-cyan-400 to-emerald-400" />
          <h1 className="text-2xl font-black text-white md:text-3xl capitalize">{title}</h1>
          {pagination && (
